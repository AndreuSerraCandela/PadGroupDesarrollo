report 51200 "Sales - Invoice-PardGroup"
{
    RDLCLayout = './Report.50200.SalesInvoicePard.rdlc';
    // WordLayout = './Report.50103.SalesInvoice.docx';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(TelefonoLbl; TelefonoLbl) { }
            column(EmailLbl; EmailLbl) { }
            column(AbonoCtaLbl; AbonoCtaLbl) { }
            column(EntidadLbl; EntidadLbl) { }
            column(DireccionCtaLbl; DireccionCtaLbl) { }
            ...
            { }
            column(TextLegal; CompanyInfo.TextoPie) { }
            ...
            column(CodigoPosta1; CompanyInfo."Post Code") { }
            column(City1; CompanyInfo.City) { }
            column(TelefonoPieLbl; TelefonoPieLbl) { }
            column(FaxPieLbl; FaxPieLbl) { }
            column(Fax; CompanyInfo."Fax No.") { }
            column(EmailCompany; CompanyInfo."E-Mail") { }
            // column(webcompany; CompanyInfo."Home Page") { }
            column(webcompany; CompanyInfo."Home Page") { }
            //

            //>>
            column(textoLibre; CompanyInfo.TextoLibre) { }
            Column(legalTextTxt; legalTextTxt) { }
            column(codOferta; Header."Cod. Oferta de Proyecto") { }
            column(CodOferta_lbl; codOfertaLbl) { }
            column(DatosFiscales_Lbl; DatosFiscalesLbl) { }
            column(Contacto_Lbl; ContactoLbl) { }
            column(ContactoLblCliente; ContactoLblCliente) { }
            ...
            column(LogoPieFactura; CompanyInfo."Logo Pie Factura") { }
            ...
            //  {
            //  }
            column(DocumentDate_Lbl; DocumentDateLbl)
            {
            ...
            column(VATAmtLblTotal; VATAmtLblTotal)
            { }
            ...
            //>>
            column(IVAVisibleMas; IVAVisibleMas)
            {
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(Description_Line_Lbl; Description_LineLbl)
                {
                }
                ...
                column(LineAmount_Line_Lbl; LineAmount_LineLbl) { }
                ...
                column(VATAmount_VatAmountLine_line; VATAmountLine."VAT Amount")
                {
                ...
                begin
                    ...
                    ///>> verificar si tiene mas IVA.
                    if CodigoIVA = '' then begin
                        CodigoIVA := VATAmountLine."VAT Identifier";
                        IVAVisibleMas := true;
                    end else
                        if CodigoIVA <> VATAmountLine."VAT Identifier" then begin
                            CodigoIVA := VATAmountLine."VAT Identifier";
                            IVAVisibleMas := false;
                        end;
                    ///>>>>>>
    ...
    begin
        GLSetup.Get();
        CompanyInfo.SetAutoCalcFields(Picture, "Logo Pie", "Invoice Legal Text", "Logo Pie Factura");
        CompanyInfo.Get();
        SalesSetup.Get();
    ...
    var
        //<<LABEL
        Quantity_Line_Lbl: Label 'Ud./Uds.';
        MostraNCuenta: Boolean;
        IVAVisibleMas: Boolean; // Esten campo identifica si tiene varios tipos de IVA
        CodigoIVA: Code[20];
        DocumentTitleLbl: Label 'Invoice Number';//INVOICE
        ClienteLbl: Label 'Customer'; //Cliente
        CIFLbl: Label 'CIF'; //Registration number
        DireccionLbl: Label 'Address'; //Dirección
        TelefonoLbl: Label 'Phone'; // Phone //Telefono
        TelefonoPieLbl: Label 'Tel:'; //telefono del pie.
        FaxPieLbl: Label 'Fax:'; //FAX del pie.
        EmailLbl: Label 'e-mail'; //e-mail
        AbonoCtaLbl: Label 'ABONO EN CUENTA'; //OWNER

        EntidadLbl: Label 'ENTIDAD'; //OWNER
        DireccionCtaLbl: Label 'DIRECCION'; // ADDRESS

        IbanLbl: Label 'IBAN'; // IBA
        DocumentDateLbl: Label 'Invoice Date'; //Fecha Factura:
        UnitPrice_Lbl: Label 'Precio'; //PRICE
        LineAmount_LineLbl: Label 'TOTAL'; //TOTAL
        Description_LineLbl: Label 'Description';//CONCEPT Description //Descripción
        ExternalDocumentNoLlb: Label 'Customer Order Number';//  No.Pedido Cliente
        DueDateLbl: Label 'Due Date';//'FECHA VTO';
        ...
        SubtotalLbl: Label 'Tax Base'; //Base Imponible
        DatosBancariosLbl: Label 'Bank Data';//'Datos Banco';

        SwiftCaptionLbl: Label 'SWIFT';
        SwiftTxt: Text;
        TotalLbl: Label 'Total';
        DatosFiscalesLbl: Label 'Tax Data'; //Datos Fiscales:
        ContactoLbl: Label 'Billing Contact:';//Contacto de Facturacion
        ContactoLblCliente: Label 'Contact'; //contacto
        VATAmtSpecificationLbl: Label 'VAT Amount Specification';
        // VATAmtLbl: Label 'VAT Amount';
        NcuentaLbl: Label 'Account Number'; //'Numero Cuenta'
        NCuentaTxt: Text;
        codOfertaLbl: Label 'Code Quote'; //Code QuoteCód. de Oferta:'
        VATAmtLbl: Label 'VAT'; //IVA
        VATAmtLblTotal: Label 'Total IVA';
        ...
        Reporfa: Report "Standard Sales - Invoice";
    ...
    begin
        ...
        FillNameValueTable(LeftHeader, Header.FieldCaption(Header."Bill-to Name"), Header."Bill-to Name");
        FillNameValueTable(LeftHeader, Header.GetCustomerVATRegistrationNumberLbl, Header.GetCustomerVATRegistrationNumber);
        // FillNameValueTable(LeftHeader, Header., Header.GetCustomerVATRegistrationNumber);
        FillNameValueTable(LeftHeader, Header."Order No.", Header."Order No.");
    ...
    // var
    //     EnvInfoProxy: Codeunit "Env. Info Proxy";
    //     reportFactura:Report "Standard Sales - Invoice";
    // begin
